...
export default function DatasetLibraryPanel({
  ...
  useEffect(() => {
    ...
    const firstInput = editorRef.current.querySelector("input:not([disabled]), select:not([disabled])");
    window.setTimeout(() => firstInput?.focus({ preventScroll: true }), 220);
  }, [selectedPublishedDataset?.id]);
