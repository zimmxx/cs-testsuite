...
const APP_TAB_IDS = new Set(RAIL_SECTIONS.flatMap((section) => section.items.map((item) => item.id)));
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return APP_TAB_IDS.has(requestedTab) ? requestedTab : "propagation";
  });
  ...
  async function handleQuickDatasetLoad(value) {
    ...
    if (source === "github") {
      const dataset = allRemoteLibraryDatasets.find((item) => String(item.id) === datasetId);
  ...
  }, [allRemoteLibraryDatasets, selectedPublishedDatasetId]);
  const loadedGithubDatasetId = selectedGithubDatasetId(quickDatasetSelection);
  const loadedGithubDataset = allRemoteLibraryDatasets.find((dataset) => dataset.id === loadedGithubDatasetId) || null;
  const selectedPublishedDataset = allRemoteLibraryDatasets.find((dataset) => dataset.id === selectedPublishedDatasetId) || null;
  ...
  const quickDatasetProjects = [...new Set(
    [...allRemoteLibraryDatasets, ...currentDatasetRows]
      .map(quickDatasetProject)
      .filter(Boolean)
  )].sort((left, right) => left.localeCompare(right, undefined, { numeric: true, sensitivity: "base" }));
  const quickRemoteDatasets = quickDatasetProjectSelection
    ? allRemoteLibraryDatasets.filter((dataset) => quickDatasetProject(dataset) === quickDatasetProjectSelection)
    : [];
...
{activeTab === "heater" ? <HeaterEfficiencyPanel sourceMeta={sourceMeta} heaterMetrics={metrics.heater} statusMessage={statusMessage} isUploading={isUploadingHeaterFiles} onFolderUpload={handleHeaterUpload} onFileUpload={handleHeaterUpload} onConfigChange={(field, value) => setSourceMeta((previous) => ({ ...previous, [field]: value }))} /> : null}
          ...
          {activeTab === "ai-diagnostics" ? (
            <AiDiagnosticsPanel
              ...
              remoteDatasets={allRemoteLibraryDatasets}
