...
export default function App() {
  ...
  const normalizedRows = useMemo(
    () => buildNormalizedRows(currentRows, currentMap, sourceMeta),
    [currentRows, currentMap, sourceMeta]
  );
  ...
  const filteredRows = useMemo(() => {
    ...
    return normalizedRows
      .filter((row) => JSON.stringify(row).toLowerCase().includes(deferredSearch.toLowerCase()))
      .slice(0, 6);
  ...
  function updateTab(tabId) {
    startTransition(() => {
      if (APP_TABS.some((tab) => tab.id === tabId)) {
        setActiveTab(tabId);
  ...
  async function handleFileUpload(event) {
    ...
    setSourceMeta({
      name: file.name,
      type: sourceTypeLabel(file.name),
      defaultMetricFamily: inferredMap.metric_family ? "propagation" : "propagation",
      defaultWavelengthNm: 1550
    });
  ...
  function exportNormalizedCsv() {
    downloadBlob(normalizedRowsToCsv(normalizedRows), "normalized-wafer-measurements.csv", "text/csv;charset=utf-8");
  }

  function exportReportJson() {
    downloadBlob(JSON.stringify(reportState, null, 2), "wafer-report-summary.json", "application/json");
  }
      ...
      <div className="dashboard-shell">
        <aside className="dashboard-rail">
          <div className="brand-mark">
            <div className="brand-wafer" />
          </div>
          {RAIL_SECTIONS.map((section) => (
            <SidebarSection key={section.title} section={section} activeTab={activeTab} onSelect={updateTab} />
          ))}
          <div className="rail-user">
            <div className="rail-avatar">SE</div>
            <div>
        ...
        <main className="dashboard-main">
          <header className="dashboard-header">
            ...
            <div className="dashboard-header-filters">
              <FilterField
                ...
                options={[projectName, "Line_220SOI_May", "Heater_Test_17"]}
              />
              <FilterField
                label="Wafer"
                value={waferName}
                onChange={setWaferName}
                options={[waferName, "WAFER_0419B", "WAFER_0312C"]}
              />
              <FilterField
                label="Date"
                value={selectedDate}
                onChange={setSelectedDate}
                options={[selectedDate, "2025-04-18", "2025-04-11"]}
                icon="Cal"
              />
              <label className="upload-measurement-button">
                <input type="file" accept=".txt,.csv,.xlsx,.xls" onChange={handleFileUpload} />
                <span>Upload Measurement Files</span>
              </label>
            </div>
          </header>
          ...
          <section className="hero-stats-row">
            <ShellStat
              label={primaryMetric.title}
              value={formatMetric(primaryMetric.key, primaryMetric.value)}
              note="Across all matched dies"
              tone="primary"
              icon={primaryMetric.icon}
            />
            <ShellStat
              label="Fit R2"
              value={(1 - (propagationLead?.mse ?? 0.013)).toFixed(3)}
            ...
            <ShellStat
              label="Devices"
              value={datasetSummary.rows.toLocaleString()}
              note={`Across ${sourceMeta.type.includes("excel") ? 1 : 2} source streams`}
              tone="mint"
              icon="Dev"
            />
            <ShellStat
              label="Wavelength"
              value={`${sourceMeta.defaultWavelengthNm} nm`}
              note="Primary extraction wavelength"
              tone="orange"
              icon="WL"
            />
            <ShellStat
              label="Sources"
              value={rawRows.length ? 1 : 2}
              note={rawRows.length ? sourceMeta.type : "TXT, XLSX"}
              tone="rose"
              icon="Src"
            />
            <ShellStat
          ...
          <section className="analysis-top-grid">
            <article className="analysis-card analysis-chart-card">
              <div className="analysis-card-head">
                ...
                <div className="analysis-card-controls">
                  <span>Model</span>
                  <select defaultValue="Linear (dB = a.L + b)">
                    <option>Linear (dB = a.L + b)</option>
                    <option>Robust linear fit</option>
                  </select>
                  <button type="button">Fit Model</button>
                </div>
              </div>
              <div className="analysis-card-body split-layout">
                ...
                <aside className="fit-results-card">
                  <h3>Fit Results</h3>
                  <ResultKeyValue label="a (slope)" value={formatMetric("propagation", propagationLead?.lossDbPerCm ?? null)} />
                  <ResultKeyValue label="b (intercept)" value={`${(propagationLead?.interceptDb ?? 0).toFixed(2)} dB`} />
                  <ResultKeyValue label="R2" value={(1 - (propagationLead?.mse ?? 0.013)).toFixed(3)} />
                  <ResultKeyValue label="RMSE" value={`${Math.sqrt(propagationLead?.mse ?? 0).toFixed(2)} dB`} />
                  <ResultKeyValue label="Fit Range" value="0.10 - 3.00 cm" />
                  <ResultKeyValue label="Unit" value={metricLabel(selectedWaferMetric)} />
                </aside>
              </div>
            </article>
            ...
            <article className="analysis-card analysis-wafer-card">
              <div className="analysis-card-head">
                <div>
                  <h2>Wafermap - {metricLabel(selectedWaferMetric)}</h2>
                </div>
              ...
              <div className="wafer-footer-bar">
                <div>
                  <span>Show</span>
                  <select defaultValue="All Dies">
                    <option>All Dies</option>
          ...
          <section className="analysis-bottom-grid">
            <article className="analysis-card wide-span">
              <div className="analysis-card-head">
                <div>
                  <h2>Normalized Dataset</h2>
                  <p>Unified CSV-ready rows from the shared translation layer.</p>
                </div>
                <div className="dataset-toolbar">
                  <input
                    value={search}
                    onChange={(event) => setSearch(event.target.value)}
                    placeholder="Search rows, chips, or devices"
                  />
                  <button type="button" onClick={exportNormalizedCsv}>
                    Export CSV
                  </button>
                </div>
              </div>
                ...
                <table>
                  ...
                  <tbody>
                    {filteredRows.map((row) => (
                      <tr key={`${row.source_name}-${row.row_index}`}>
                        <td>{row.chip_id || "--"}</td>
                        <td>{waferName}</td>
                        <td>{row.die_x ?? "--"}</td>
                        <td>{row.die_y ?? "--"}</td>
                        <td>{row.source_type.includes("excel") ? "XLSX" : row.source_type.includes("txt") ? "TXT" : row.source_type}</td>
                        <td>{row.relative_length_mm ?? "--"}</td>
                        <td>{row.transmission_db ?? "--"}</td>
                        <td>{metrics.propagation.byChip.find((item) => item.chipId === row.chip_id)?.lossDbPerCm.toFixed(2) ?? "--"}</td>
                        <td>{row.wavelength_nm ?? sourceMeta.defaultWavelengthNm}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <article className="analysis-card">
              ...
              <TranslationStatus
                ...
                totalRows={datasetSummary.rows}
                matchedDevices={matchedDevices}
                unmatchedDevices={unmatchedDevices}
              />
              <button type="button" className="secondary-action" onClick={loadDemo}>
            ...
            <article className="analysis-card">
              <div className="analysis-card-head">
                <div>
                  ...
                  <p>Export-ready representation of wafer quality.</p>
                </div>
                <button type="button" onClick={exportReportJson}>
                  Open Report
                </button>
              </div>
              <ReportPreviewCard reportState={reportState} selectedMetricLabel={metricLabel(selectedWaferMetric)} />
            </article>
          </section>

          {activeTab === "intake" ? (
            <section className="mapping-drawer">
              <div className="mapping-drawer-head">
                <div>
                  <h2>Translator Mapping</h2>
                  <p>Review how the unified CSV schema is derived from your measurement file.</p>
                </div>
                <div>
                  <span>Canonical export columns</span>
                  <small>{requiredColumns().slice(0, 6).join(", ")}, ...</small>
                </div>
              </div>
              <div className="mapping-grid">
                {[
                  ...
                  <MappingSelect
                    ...
                    onChange={(nextValue) => setColumnMap((prev) => ({ ...prev, [field]: nextValue }))}
                  />
                ))}
              </div>
              <div className="mapping-family-row">
                <FilterField
                  label="Default metric family"
                  value={sourceMeta.defaultMetricFamily}
                  onChange={(value) => setSourceMeta((prev) => ({ ...prev, defaultMetricFamily: value }))}
